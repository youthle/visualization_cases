...
function draw_map (map_dim, map_projection, map_path, map_svg_id, palette, json_data, x_var, y_var) {	
	...
	$.getJSON(spatial, function (json) {
		//console.log(json.objects.unnamed);

		//set the projection 
		var geometries = json.objects.unnamed.geometries;
		set_projection(map_projection, map_path, geometries, map_dim.width, map_dim.height);
		...
		geo_entity.selectAll("path")
			...
			.data(geometries.filter(function (d) {  
				console.log(d.properties.ADM0_A3.length);
				return d.properties.ADM0_A3.length > 0; }))
			.enter().append("path")
			.attr("d", map_path)
			.style("fill", function(d) { 
				var test_hdata = $.map(map_data.filter(function (dd) { 
					//console.log(dd.CountryCode);
					return dd.CountryCode == d.properties.ADM0_A3; }), function (g) { return g; });
			...
			$.getJSON(boroJSON, function (b_json) {
				...
				geo_entity_hover.selectAll("path")
					.data(json.features.filter(function (d) { return d.ADM0_A3.length > 0; }))
					.enter().append("path")
						.attr("class", "UHF_out")
						.attr("id", function (d) { return "UHF_" + d.ADM0_A3; })
						.attr("d", map_path)
						.attr("title", function(d) { return d.properties.ADMIN; })
						.on("mouseover", function(d,i){
							d3.selectAll("#pt_" + d.ADM0_A3).style("fill", "rgb(0,255,255)").attr("r", 5);	
							//d3.select(this).attr("class", "UHF_over");
							d3.selectAll("#UHF_" + d.ADM0_A3).attr("class", "UHF_over");
						})
						.on("mouseout", function(d){
							d3.selectAll("#pt_" + d.ADM0_A3).style("fill", "rgb(0,0,0)").attr("r", 3);
							//d3.select(this).attr("class", "UHF_out");
							d3.selectAll("#UHF_" + d.ADM0_A3).attr("class", "UHF_out");
						});
